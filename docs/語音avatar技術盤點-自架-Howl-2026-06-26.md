# 沐寧 Munea · 會說話的臉 — 自架技術 + 開源地景盤點（第一階段）
...
**🔴 致命細節 2（授權地雷）**：`Duix-Mobile` 的 license **不是 Apache**、是 custom license、**1000 MAU 就觸天花板**。這比 `Duix-Avatar` 的「10萬用戶」嚴格 100 倍。沐寧若用 Duix-Mobile 當即時引擎、**過 1000 個長輩用戶就要回求授權**——商業模式上是隱藏地雷，卡西法做架構時要把這條當紅線標出來。
...
**霍爾判斷**：**這軌技術風險最低、硬體成本趨近於零、且天然避開我們踩過的 viseme 整圖切換牆。** 對沐寧的長輩場景——可愛動物比「擬真人臉」更沒有恐怖谷風險、長輩接受度可能更高（參考日本介護 PARO 海豹機器人邏輯）。
...
| 方案 | 紅線 |
|---|---|
| **Duix-Mobile** | 🔴 custom license、**1000 MAU 天花板**、超過要回求授權 + 須標「Powered by Duix.com」+ 模型名要加「duix.com」。**沐寧過 1000 長輩就觸雷。** |
